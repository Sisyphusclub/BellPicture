...
- Provider routing is separate from API authorization: standard requests use
...
- Persisted history records the provider-selected model when the provider
  returns one, so configured high-resolution model overrides are visible in
  history.
...
- Good: an admin posts `{ prompt, aspectRatio: "16:9", resolution: "4k" }` to
  `/api/images/generate/high-res`; the provider receives `size: "3840x2160"` at
  the configured high-resolution origin, and history persists the returned
  width, height, and provider-selected model.
