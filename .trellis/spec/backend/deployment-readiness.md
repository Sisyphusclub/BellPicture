...
- Trigger: a public deployment must distinguish process liveness from storage
  readiness, identify the deployed release, trust only configured proxy hops,
  keep the backend private to the Compose network, and expose the SPA and API
  through one origin.
...
APP_VERSION: string; // release tag or commit, local default 0.0.0
```
...
- `/api/health/live` returns `200 { status, uptimeSec, version }` when the
  process can serve requests. It does not touch external dependencies, and
  `version` equals the configured `APP_VERSION`.
...
- `TRUST_PROXY_HOPS=0` is the safe default. Increase it only to the exact number
  ...
  `X-Forwarded-For`.
- Base Compose requires `APP_VERSION`; set it to the immutable release tag or
  commit so health and incident evidence identify the running artifact.
...
| Base Compose omits `APP_VERSION`              | Compose interpolation fails     |
| `APP_VERSION` is configured                   | Health returns the same value   |
...
- Bad: leaving production health at `0.0.0` makes instances indistinguishable
...
- Env tests assert `TRUST_PROXY_HOPS` accepts zero/positive integers and rejects
  invalid values.
- Env and route tests assert `APP_VERSION` defaults locally, accepts a release
  identifier, and is returned unchanged by liveness/readiness responses.
...
backend:
  expose: ["3000"]
  environment:
    APP_VERSION: ${APP_VERSION:?Set APP_VERSION to the release tag or commit}
  healthcheck:
    test:
      [
        "CMD-SHELL",
