...
* [ ] 两页的层级、间距和控件密度符合 beUI 工作台风格及项目设计规范。
* [ ] 资产页所有筛选、视图切换、单项与批量操作保持可用。
* [ ] 用户管理页创建、搜索、分页、额度保存、刷新与删除保护保持可用。
* [ ] 资产媒体保持页面主视觉，用户表格可快速比较身份、用量和额度。
* [ ] 1920px、1440px、1024px、390px 视口无重叠、裁切或横向溢出。
* [ ] 模板、资产和用户管理页面在相同视口下具有一致的外框左右边界与响应式水平边距。
* [ ] 键盘、焦点、ARIA、禁用/等待和 reduced-motion 行为不回退。
* [ ] 相关前端测试、Lint、TypeScript 和生产构建通过。
