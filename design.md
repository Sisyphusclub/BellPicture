...
| Discovery Sidebar       | [shadcn Sidebar](https://ui.shadcn.com/docs/components/sidebar)          | `src/components/ui/sidebar.tsx` and `LandingSidebar`                                    | Route items, current state, tooltips, mobile sheet, Nebulens logo, account, and mobile check-in        |
| Morphing Popover        | Existing beUI morph interaction adapter                                  | `src/components/motion/popover-morph.tsx` and `LandingAccountActions`                   | Notifications, daily check-in, account details, outside click, Escape, and reduced motion              |
...
| Image Gallery Vertical  | [image-galleries](https://pro.beui.dev/components/image-galleries)       | `src/components/premium/image-galleries/image-gallery-vertical.tsx`                     | Six product-owned images, static waterfall columns, responsive column count, and image-detail actions  |
...
- Use a full dark first viewport with the registry-copied shadcn Sidebar, a centered product statement in the remaining canvas, and a functional beUI Pro Agent Composer. The hero video is a true viewport layer (`inset: 0`, `100% x 100%`, `object-fit: cover`) rather than an enlarged content asset. Keep the desktop Sidebar in its standard icon-collapsed state, use `SidebarMenuButton` for every route, separate creation and asset groups with whitespace rather than a visible divider, and use component-owned tooltips for collapsed labels. Product code may compose route data and account actions but must not rewrite Sidebar state, Sheet, Tooltip, or menu-button internals.
- Keep desktop account actions as a compact upper-right cluster ordered as Templates, Notifications, Personal Credits, and Account. Use quiet `40px` icon buttons, one `44px` graphite credits pill with a divider and sparkle/count, and a `40px` circular account trigger. Do not reintroduce a large login bar or duplicate this cluster inside the hero content.
- The credits pill opens a morphing daily check-in popover below the trigger. Before claiming, show `赢取每日灵感值！`, the configured reward, and one bright `签到` action; after claiming, switch to `今日灵感已领取` and a disabled `已签到` state. The displayed credits and composer quota update from the same server response. Guests are routed into authentication instead of receiving a simulated reward.
- Daily check-in is functional product state, not decorative copy. It grants the server-configured reward once per `Asia/Shanghai` calendar day, survives reloads, and contributes to the same remaining credits consumed by image generation.
...
- Hide the four-control desktop account cluster below `860px`; expose account identity, current credits, and the same check-in action inside the mobile Sidebar so the feature remains available without compressing the hero header.
...
- Present the six bundled works with beUI Pro `image-gallery-vertical`: four static waterfall columns on wide screens and the component's compact two-column layout below the large breakpoint. Render each local image once per responsive gallery, keep the edge fade and image-detail action, and do not add autoplay, duplicated loop tracks, pagination, or previous/next controls.
- Keep the home gallery unlabeled and pull it upward into the lower hero space while preserving a clear visual boundary below the Agent Composer. Blend its static columns into the graphite canvas with a long, symmetric multi-stop edge mask instead of a short linear fade. Use real generated images with `16px` radius, subtle borders, a restrained hover zoom, and the shared image-detail modal on primary images.
