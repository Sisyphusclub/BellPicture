...
describe('template detail sizing contract', () => {
  it('uses a wide, viewport-bounded desktop preview with a readable detail rail', () => {
    ...
    expect(desktopModalRule).toContain('width: calc(100% - 32px) !important;');
    expect(desktopModalRule).toContain('max-width: 1360px !important;');
    expect(mobileStart).toBeGreaterThanOrEqual(0);
    expect(mobileModalRule).toContain('width: 100% !important;');
    expect(contract).toContain('@media (min-width: 861px)');
    expect(contract).toContain('height: min(760px, calc(100svh - 32px));');
    expect(contract).toContain('grid-template-columns: minmax(0, 1fr) minmax(400px, 0.48fr);');
