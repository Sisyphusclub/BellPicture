...
describe('AppHeader', () => {
  ...
  it('renders the left navigation items in the requested order', async () => {
    ...
    expect(wrapper.get('.sidebar-brand__mark').attributes('src')).toBe('/brand/logo.png');
    expect(wrapper.get('.sidebar-brand__mark').attributes('alt')).toBe('Nebulens 标志');
    expect(wrapper.get('.sidebar-brand').attributes('aria-label')).toBe('Nebulens，返回发现首页');
    ...
    expect(wrapper.html()).toContain(
      'M4 7.5A2.5 2.5 0 0 1 6.5 5H10l2 2.5h5.5A2.5 2.5 0 0 1 20 10v6.5A2.5 2.5 0 0 1 17.5 19h-11A2.5 2.5 0 0 1 4 16.5Z',
    );
  ...
  it('keeps the Nebulens logo visually compact inside the sidebar button', async () => {
