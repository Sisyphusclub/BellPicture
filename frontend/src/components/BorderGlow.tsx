...
function buildGradientVars(colors: string[]): Record<string, string> {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const color = palette[Math.min(colorIndex, palette.length - 1)] ?? DEFAULT_COLORS[0];
    const key = GRADIENT_KEYS[i];
    const position = GRADIENT_POSITIONS[i];
    if (!key || !position || !color) continue;

    vars[key] = `radial-gradient(at ${position}, ${color} 0px, transparent 50%)`;
  }

  vars['--gradient-base'] = `linear-gradient(${palette[0] ?? DEFAULT_COLORS[0]} 0 100%)`;
  return vars;
}
...
function BorderGlow({
  ...
  const getCenterOfElement = useCallback((element: HTMLElement): [number, number] => {
  ...
  const glowStyle: BorderGlowStyle = {
